...
/** DMSubviewConstraint allows you to set custom constraint for a DSplitView subview object. You can specify if subview can be collapsed and minimum & maximum allowed size. Default DSplitView implementation does not set any constraint */

@interface DSubviewConstraint : NSObject { }
...
@implementation DSubviewConstraint
...
#pragma mark - DSplitView Implementation

@interface DSplitView() {
...
- (void) initializeSplitView {
    self.delegate = self;
    self.subviewsResizeMode = DSplitViewResizeModeProportional;
    ...
    for (NSUInteger k = 0; k < self.subviews.count; k++)
        [subviewContraints addObject:[[DSubviewConstraint alloc] init]];
}
...
- (void) setMaxSize:(CGFloat) maxSize ofSubviewAtIndex:(NSUInteger) subviewIndex {
    ((DSubviewConstraint*)subviewContraints[subviewIndex]).maxSize = maxSize;
}

- (void) setMinSize:(CGFloat) minSize ofSubviewAtIndex:(NSUInteger) subviewIndex {
    ((DSubviewConstraint*)subviewContraints[subviewIndex]).minSize = minSize;
}

- (CGFloat) minSizeForSubviewAtIndex:(NSUInteger) subviewIndex {
    return ((DSubviewConstraint*)subviewContraints[subviewIndex]).minSize;
}

- (CGFloat) maxSizeForSubviewAtIndex:(NSUInteger) subviewIndex {
    return ((DSubviewConstraint*)subviewContraints[subviewIndex]).maxSize;
}

- (void) setCanCollapse:(BOOL) canCollapse subviewAtIndex:(NSUInteger) subviewIndex {
    ((DSubviewConstraint*)subviewContraints[subviewIndex]).canCollapse = canCollapse;
}

- (BOOL) canCollapseSubviewAtIndex:(NSUInteger) subviewIndex {
    return ((DSubviewConstraint*)subviewContraints[subviewIndex]).canCollapse;
}
...
- (CGFloat)splitView:(NSSplitView *)splitView constrainMinCoordinate:(CGFloat)proposedMin ofSubviewAt:(NSInteger)viewIndex {
    DSubviewConstraint *subviewConstraint = ((DSubviewConstraint*)subviewContraints[viewIndex]);
...
- (CGFloat)splitView:(NSSplitView *)splitView constrainMaxCoordinate:(CGFloat)proposedMax ofSubviewAt:(NSInteger)offset {
    DSubviewConstraint *constraintShrinkingSubview = ((DSubviewConstraint*)subviewContraints[offset+1]);
    DSubviewConstraint *constraintGrowingSubview = ((DSubviewConstraint*)subviewContraints[offset]);
...
- (void)splitView:(NSSplitView *)splitView resizeSubviewsWithOldSize:(NSSize)oldSize {
    ...
    switch (self.subviewsResizeMode) {
        case DSplitViewResizeModeUniform: // UNIFORM RESIZE
            [self applyUniformResizeFromOldSize:oldSize];
            break;
        case DSplitViewResizeModePriorityBased: // PRIORITY BASED RESIZE
            [self applyPriorityResizeFromOldSize:oldSize];
            break;
        case DSplitViewResizeModeProportional: // PROPORTIONAL RESIZE
        default:
...
- (DSubviewConstraint *) constraintForSubview:(NSView *) subview {
...
- (void) applyUniformResizeFromOldSize:(CGSize) splitViewOldSize {
    ...
    while (fabs(deltaValue) > 0.5f) {
        ...
        [subviewContraints enumerateObjectsUsingBlock:^(DSubviewConstraint *constraint, NSUInteger subviewIndex, BOOL *stop) {
...
- (void) applyPriorityResizeFromOldSize:(CGSize) splitViewOldSize {
    ...
    for (NSNumber *priorityIndex in [[priorityIndexes allKeys] sortedArrayUsingSelector:@selector(compare:)]) {
        ...
        if (![self isSubviewCollapsed:subview]) {
            ...
            DSubviewConstraint *constraint = [self constraintForSubview:subview];
...
- (void) calculateResizableSubviews:(BOOL *) subviewCanBeResized
    ...
    [subviewContraints enumerateObjectsUsingBlock:^(DSubviewConstraint* constraint, NSUInteger viewIndex, BOOL *stop) {
...
- (BOOL) setPositions:(NSArray *)newPositions ofDividersAtIndexes:(NSArray *)indexes animated:(BOOL) animated completitionBlock:(void (^)(BOOL isEnded)) completition {
	...
	if (animated) {
		if ([self.eventsDelegate respondsToSelector:@selector(splitView:splitViewIsAnimating:)])
			[((id <DSplitViewDelegate>)self.delegate) splitView:self splitViewIsAnimating:YES];
        ...
        [NSAnimationContext runAnimationGroup:^(NSAnimationContext *context) {
            ...
            if ([self.eventsDelegate respondsToSelector:@selector(splitView:splitViewIsAnimating:)])
                [((id <DSplitViewDelegate>)self.delegate) splitView:self splitViewIsAnimating:NO];
        }];
	} else {
        NSRect newRect[numberOfSubviews];
        [self getNewSubviewsRects:newRect withIndexes:indexes andPositions:newPositions];
        ...
        if ([self.eventsDelegate respondsToSelector:@selector(splitView:subview:stateChanged:)]) {
            if (isCollapsing)
                [self.eventsDelegate splitView:self
                                  ...
                                  stateChanged:DSplitViewStateCollapsed];
            if (isExpanding)
                [self.eventsDelegate splitView:self
                                       subview:dividerIndex
                                  stateChanged:DSplitViewStateExpanded];
        }
    }
